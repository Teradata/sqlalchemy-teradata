...
from sqlalchemy import pool, Table, Column, Index
from sqlalchemy import types as sqltypes
from sqlalchemy.engine import default
from sqlalchemy.sql import select, and_, or_
from sqlalchemy.sql.expression import text, column, asc
...
class TeradataDialect(default.DefaultDialect):
    ...
    def create_connect_args(self, url):
        if url is not None:
            ...
            cparams = {p: params[p]
                       for p in params
                       if p not in ['host', 'username', 'password']}
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        stmt = select([column('tablename')],
            ...
            .where(and_(text('DatabaseName (NOT CASESPECIFIC) = '
                             'CAST(:schema as VARCHAR(128)) '
                             '(NOT CASESPECIFIC)'),
                        text('TableName=:table_name')))

        res = connection.execute(
            stmt, schema=schema, table_name=table_name).fetchone()
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        stmt = select(['*'], from_obj=text(dbc_columninfo)).where(
            and_(text('DatabaseName (NOT CASESPECIFIC) = '
                      'CAST(:schema as VARCHAR(128)) (NOT CASESPECIFIC)'),
                 text('TableName=:table_name')))

        res = connection.execute(
            stmt, schema=schema, table_name=table_name).fetchall()
    ...
    def get_view_definition(self, connection, view_name, schema=None, **kw):
        ...
        res = connection.execute('show table {}.{}'.format(schema, view_name))
        return self.normalize_name(res.scalar())
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        # K for Primary Key
        res = connection.execute(
            stmt, schema=schema, table=table_name, indextype='K').fetchall()
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        # U for Unique
        res = connection.execute(
            stmt, schema=schema, table=table_name, indextype='U').fetchall()
        ...
        for constraint_info, constraint_cols in groupby(res, grouper):
            ...
            for constraint_col in constraint_cols:
                unique_constraint['column_names'].append(
                    self.normalize_name(constraint_col['ColumnName']))
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        stmt = select([column('IndexID'), column('IndexName'),
        ...
        def grouper(fk_row):
            return {
                #ID if IndexName is None
                'name': fk_row.IndexName or fk_row.IndexID,
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        def grouper(fk_row):
            return {
                # TODO: Check what to do if IndexName is None
                'name': fk_row.IndexName or fk_row.IndexNumber,
        ...
        for index_info, index_cols in groupby(res, grouper):
            ...
            for index_col in index_cols:
                index_dict['column_names'].append(
                    self.normalize_name(index_col['ColumnName']))
    ...
    def conn_supports_autocommit(self, connection, **kw):
        """Returns True if autocommit is used for this connection (underlying
        Teradata session) else False.
        """
